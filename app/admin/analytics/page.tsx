...
export default function AdminAnalytics() {
  ...
  const labelStyle = { fontFamily:'var(--font-mono),monospace', fontSize:11, color:'var(--muted)', letterSpacing:'.08em', textTransform:'uppercase' as const }
  const inputStyle = { background:'rgba(255,255,255,.04)', border:'1px solid var(--line2)', borderRadius:8, padding:'8px 12px', fontSize:13, color:'var(--text)', fontFamily:'var(--font-jakarta),sans-serif', outline:'none' }

  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'48px 32px 100px' }}>
      <div style={{ marginBottom:36 }}>
        <div style={{ ...labelStyle, marginBottom:12 }}>/ Analytics</div>
        <h1 style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, fontSize:'clamp(28px,4vw,44px)', letterSpacing:'-.03em', margin:'0 0 8px' }}>Analytics</h1>
      ...
      <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:14, padding:20, marginBottom:28, display:'flex', flexWrap:'wrap', gap:14, alignItems:'flex-end' }}>
        ...
        <button onClick={load} style={{ background:'rgba(255,255,255,.06)', border:'1px solid var(--line2)', borderRadius:999, padding:'9px 20px', fontSize:13, fontWeight:500, color:'var(--text)', fontFamily:'var(--font-jakarta),sans-serif', cursor:'pointer' }}>Filter</button>
        <button onClick={handleExport} disabled={exporting} style={{ background:'var(--mint)', color:'#06160E', border:'none', borderRadius:999, padding:'9px 20px', fontSize:13, fontWeight:600, fontFamily:'var(--font-jakarta),sans-serif', cursor:'pointer', marginLeft:'auto' }}>
          {exporting ? 'Exporting…' : '↓ Export CSV'}
        </button>
      </div>

      {loading
        ? <div style={{ textAlign:'center', padding:'60px 0', color:'var(--muted)', fontFamily:'var(--font-mono),monospace', fontSize:12 }}>Loading…</div>
        : <>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:32 }}>
              {[['Total Plays', data?.totalPlays||0], ['Total Visits', data?.totalVisits||0], ['Unique Visitors', data?.totalUniqueVisitors||0], ['Avg Watch', `${Math.floor((data?.averageWatchTimeSeconds||0)/60)}m`]].map(([l,v]) => (
                <div key={l as string} style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:14, padding:'20px 22px' }}>
                  ...
                  <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontSize:30, fontWeight:800, color:'var(--mint)', letterSpacing:'-.03em' }}>{v}</div>
                </div>
              ))}
            </div>
              ...
              : <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:16, overflow:'hidden' }}>
                  ...
                  {data.videosStats.map((s:any, i:number) => (
                    <div key={s.videoId} style={{ display:'grid', gridTemplateColumns:'1fr auto auto auto auto auto', gap:16, alignItems:'center', padding:'18px 24px', borderBottom: i<data.videosStats.length-1 ? '1px solid var(--line)' : 'none' }}>
                      <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:700, fontSize:13, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.videoTitle}</div>
                      <span style={{ fontFamily:'var(--font-mono),monospace', fontSize:10, padding:'3px 9px', borderRadius:6, background: s.section==='boys' ? 'rgba(59,130,246,.1)' : 'rgba(236,72,153,.1)', color: s.section==='boys' ? '#60a5fa' : '#f472b6' }}>{s.section}</span>
                      <span style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, color:'var(--mint)', fontSize:16 }}>{s.plays}</span>
                      <span style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, color:'var(--mint)', fontSize:16 }}>{s.visits}</span>
                      <span style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, color:'var(--mint)', fontSize:16 }}>{s.uniqueVisitors}</span>
                      <span style={{ fontFamily:'var(--font-mono),monospace', fontSize:11, color:'var(--muted)' }}>{Math.floor(s.avgWatchTimeSeconds/60)}m</span>
                    </div>
                  ))}
                </div>
            }
