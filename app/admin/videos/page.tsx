    ...
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'48px 32px 100px' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:40 }}>
        <div>
          ...
          <h1 style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, fontSize:'clamp(28px,4vw,44px)', letterSpacing:'-.03em', margin:'0 0 8px' }}>Video Management</h1>
          ...
          : <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:16, overflow:'hidden' }}>
              ...
              {videos.map((v, i) => (
                <div key={v.id} style={{ display:'grid', gridTemplateColumns:'1fr auto auto auto auto', gap:16, alignItems:'center', padding:'18px 24px', borderBottom: i<videos.length-1 ? '1px solid var(--line)' : 'none', transition:'.15s' }}>
                  <div>
                    <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:700, fontSize:14, color:'var(--text)', marginBottom:3, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{v.title}</div>
                    <div style={{ fontFamily:'var(--font-mono),monospace', fontSize:10, color:'var(--dim)' }}>ID: {v.id.slice(0,8)}…</div>
                  </div>
                  ...
                  <div style={{ display:'flex', gap:10 }}>
                    ...
                    <button onClick={() => handleDelete(v.id, v.title)} style={{ fontSize:12, color:'var(--rose)', background:'none', border:'none', cursor:'pointer', padding:0, fontFamily:'var(--font-jakarta),sans-serif' }}>Delete</button>
                  </div>
                </div>
              ))}
            </div>
