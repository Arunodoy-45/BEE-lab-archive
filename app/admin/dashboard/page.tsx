  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'48px 32px 100px' }}>
      <div style={{ marginBottom:40 }}>
        <div style={{ fontFamily:'var(--font-mono),monospace', fontSize:11, color:'var(--mint)', letterSpacing:'.12em', textTransform:'uppercase', marginBottom:12 }}>/ Dashboard</div>
        <h1 style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, fontSize:'clamp(28px,4vw,44px)', letterSpacing:'-.03em', margin:'0 0 8px' }}>Overview</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:40 }}>
        {stats.map(s => (
          <div key={s.label} style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:14, padding:'20px 22px' }}>
            <div style={{ fontSize:11, fontFamily:'var(--font-mono),monospace', color:'var(--muted)', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:10 }}>{s.label}</div>
            <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontSize:32, fontWeight:800, color:'var(--mint)', letterSpacing:'-.03em' }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:40 }}>
        <div style={{ background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:16, padding:24 }}>
          <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:700, fontSize:16, marginBottom:20 }}>Top Videos by Plays</div>
          {data.topVideos.length === 0
            ...
            : data.topVideos.map((v:any,i:number) => (
                <div key={v.videoId} style={{ display:'flex', alignItems:'center', gap:14, paddingBottom:14, marginBottom:14, borderBottom: i<data.topVideos.length-1 ? '1px solid var(--line)' : 'none' }}>
                  <div style={{ fontFamily:'var(--font-mono),monospace', fontSize:11, color:'var(--dim)', minWidth:20 }}>{String(i+1).padStart(2,'0')}</div>
                  ...
                  <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:800, fontSize:18, color:'var(--mint)' }}>{v.plays}</div>
                </div>
              ))
          }
        </div>
          ...
          {quickActions.map(a => (
            <Link key={a.href} href={a.href} style={{ textDecoration:'none', display:'block', background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:14, padding:'20px 22px', transition:'.2s', color:'inherit' }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div>
                  <div style={{ fontFamily:'var(--font-jakarta),sans-serif', fontWeight:700, fontSize:14, color:'var(--text)', marginBottom:3 }}>{a.title}</div>
