...
export default function UploadVideo() {
  ...
  const [form, setForm] = useState({ title:'', description:'', category: CATS[0], section:'boys', githubUrl:'', thumbnailUrl:'' })
  ...
  return (
    <div style={{ maxWidth:680, margin:'0 auto', padding:'48px 32px 100px' }}>
      ...
      <div style={{ fontFamily:'JetBrains Mono,monospace', fontSize:11, color:'var(--mint)', letterSpacing:'.12em', textTransform:'uppercase', marginBottom:12 }}>/ New Video</div>
      <h1 style={{ fontFamily:'Syne,sans-serif', fontWeight:800, fontSize:36, letterSpacing:'-.03em', margin:'0 0 32px' }}>Add Video</h1>
      ...
      <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:22 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          <div>
            ...
            <select required value={form.category} onChange={e=>inp('category',e.target.value)} style={{ ...inputStyle, cursor:'pointer' }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11, color:'var(--dim)', marginTop:6, fontFamily:'JetBrains Mono,monospace' }}>Use the "Raw" link from GitHub for the video file</div>
        </div>
        <div>
        ...
        <div style={{ display:'flex', gap:12 }}>
          <button type="submit" disabled={loading} style={{ background:'var(--mint)', color:'#06160E', border:'none', borderRadius:999, padding:'13px 26px', fontSize:14, fontWeight:600, fontFamily:'Onest,sans-serif', cursor: loading ? 'not-allowed':'pointer', opacity: loading ? .5:1, transition:'.18s' }}>
      ...
      <div style={{ marginTop:48, background:'var(--bg2)', border:'1px solid var(--line)', borderRadius:14, padding:22 }}>
        <div style={{ fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:14, marginBottom:14, color:'var(--text)' }}>📂 Where to upload videos in GitHub</div>
        <ol style={{ fontSize:13, color:'var(--muted)', paddingLeft:18, lineHeight:2 }}>
          <li>Push <code style={{ fontFamily:'JetBrains Mono,monospace', color:'var(--mint)', fontSize:11 }}>.mp4</code> files into the <code style={{ fontFamily:'JetBrains Mono,monospace', color:'var(--mint)', fontSize:11 }}>/videos/</code> folder of this repo</li>
          <li>Navigate to the file on GitHub → click <b style={{ color:'var(--text)' }}>Raw</b></li>
          <li>Copy the URL and paste it above</li>
        </ol>
        <div style={{ fontFamily:'JetBrains Mono,monospace', fontSize:11, color:'var(--dim)', marginTop:10, background:'rgba(255,255,255,.03)', borderRadius:8, padding:'10px 14px', wordBreak:'break-all' as const }}>
          https://raw.githubusercontent.com/Arunodoy-45/BEE-lab-archive/main/videos/session-01.mp4
        </div>
      </div>
    </div>
  )
